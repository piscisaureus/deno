...
int32_t control_buffer[1];

TEST(LibDenoTest, InitializesCorrectly) {
  Deno* d = deno_new(nullptr, nullptr, control_buffer, sizeof control_buffer);
...
TEST(LibDenoTest, SendRecvSlice) {
  ...
  Deno* d = deno_new(
      ...
      [](auto deno, auto buf) {
        ...
        // because deno_set_response() takes ownership over the buffer, but we
        // are not given ownership of `buf` by our caller.
...
TEST(LibDenoTest, SendNullAllocPtr) {
  static int count = 0;
  Deno* d = deno_new(nullptr, [](auto _, auto buf) { count++; }, control_buffer,
